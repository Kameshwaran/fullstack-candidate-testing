...
const filterProcessors = {
  ...
  department: (item, values) => values.filter(value => item.department.includes(value)).length > 0,
};
...
const sort = (items, options) => {
  ...
  if (attr === 'experience') {
    return items.sort(
      (e1, e2) => experienceWeightage[e1[attr]] > experienceWeightage[e2[attr]] ? rank[0] : rank[1]
    );
  }
...
export default async (req, res) => {
  ...
  if (keyword && keyword.length > 0) {
    ...
    results.forEach((job) => {
      const matchedItems = job.items.filter((item) => (
        item.name.toLowerCase().includes(downcasedKeyword) ||
        item.job_title.toLowerCase().includes(downcasedKeyword) ||
        item.required_skills.join(' ').toLowerCase().includes(downcasedKeyword) ||
  ...
  /**
   * SEARCH BY KEYWORD ENDS
   */

  /**
   * FILTERS PROCESSING STARTS
   */
  results = Object.keys(filters)
    .reduce((resultSet, filterType) => {
      ...
      resultSet.forEach(
        (job) => {
          const matchingJobs = job.items.filter((item) => filterProcessors[filterType](item, filterValues));
